...
Áp dụng cho cả `terraform-modules` và `{org}-{service}-infra`.

| Cài đặt | Giá trị |
|---|---|
| Require a pull request before merging | ✅ Bật |
| Required approvals | 1 người trở lên |
| Require status checks to pass | ✅ Chỉ định `terraform-plan` là check bắt buộc |
| Restrict who can push | Chỉ SRE (cấm push trực tiếp) |
...
Để GitHub Actions của infra repository có thể tham chiếu `terraform-modules`, cần mở quyền truy cập.
...
両リポジトリ（`terraform-modules`・`{org}-{service}-infra`）の `main` ブランチに設定します。

| 設定項目 | 値 |
|---|---|
| Require a pull request before merging | ✅ 有効 |
| Required approvals | 1名以上 |
| Require status checks to pass | ✅ `terraform-plan` を必須チェックに指定 |
| Restrict who can push | SRE のみ（直接プッシュ禁止） |

`Settings → Branches → Branch protection rules` から設定します。

#### GitHub Environments（prod 承認フロー）

infra リポジトリの `prod` 環境に手動承認を設定し、`apply` の誤実行を防ぎます。
...
infra リポジトリの GitHub Actions が `terraform-modules` を参照できるよう、アクセス権を開放します。

`terraform-modules` リポジトリ → `Settings → Actions → Access` → **「Accessible from repositories in the organization」** を選択します。

#### GitHub Actions ワークフロー

各 infra リポジトリに以下の2つのワークフローを配置します。

**① `terraform-plan.yml`（PR 時に自動実行）**
...
**② `terraform-apply.yml`（main マージ後に自動実行）**
...
IAM / リソース変更を PR で提出（案件 infra repo）
  ↓
GitHub Actions が terraform-plan.yml を自動実行
  ↓
...
main にマージ
  ↓
GitHub Actions が terraform-apply.yml を自動実行
  ↓
